...
def generate_plan():
    ...
    prompt = f"""
You are an AI growth director and brand strategist for small businesses. Given the info below, produce a JSON object
(no markdown, no backticks) with exactly these keys:
- "ad_copy": array of 3 short ad copy variations
- "budget_split": object mapping channel names to percentage numbers that sum to 100
- "trend_summary": a short paragraph (2-3 sentences) summarizing likely marketing trends relevant to this industry
- "brand_positioning": a short paragraph (2 sentences) suggesting a brand positioning angle for this business
...
Keep every field concise. Respond with valid JSON only.
"""
...
PYEOF
git add main.py
git commit -m "Add brand positioning field"
git push
